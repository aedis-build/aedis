...
/// <summary>
///     Sessão transacional sobre uma <see cref="DbConnection" /> (SqlConnection), com acesso via Dapper. Expõe
///     a conexão e a transação subjacentes (<see cref="GetConnection" />/<see cref="GetTransaction" />) para o
///     caminho de alta performance do SqlBulkCopy. No descarte sem commit/rollback, faz rollback automático.
/// </summary>
public sealed class UnitOfWork(DbConnection connection, bool isReadOnly, ILogger<UnitOfWork> logger) : IUnitOfWork
{
    ...
    /// <summary>Expõe a conexão subjacente para operações específicas do provider (ex.: SqlBulkCopy).</summary>
    public DbConnection GetConnection() {
        EnsureOpen();
        return _connection;
    }

    /// <summary>Expõe a transação ativa (ou <c>null</c>) — exigida pelo SqlBulkCopy quando há transação aberta.</summary>
