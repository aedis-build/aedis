...
/// <summary>
///     Health check de <em>readiness</em> do SQL Server: executa <c>SELECT 1</c> em uma sessão de leitura
...
public sealed class SqlServerHealthCheck : IHealthCheck
{
    ...
    public SqlServerHealthCheck(IUnitOfWorkFactory factory, IOptions<DatabaseOptions> options,
    ...
    private async Task<HealthCheckResult> ProbeAsync(CancellationToken ct) {
        try {
            ...
            return value == 1
                ? HealthCheckResult.Healthy("Conexão SQL Server saudável.")
                : HealthCheckResult.Unhealthy("Resposta inesperada do SQL Server.");
        }
        catch (Exception ex) {
            _logger.LogError(ex, "Health check do SQL Server falhou.");
            return HealthCheckResult.Unhealthy("Conexão SQL Server indisponível.", ex);
        }
    }
}
